            ...
            if(res.status == true){
                ...
                res.collection.forEach(element => {
                    if(element.quantity > 0){
                        $("#" + $productItem).append("<option value='"+element.id+"'>"+element.name+" ("+element.sku+")"+"</option>");
                    }                    
                });
            }
        }
...
function productSKU(){
    ...
    if(product_name != ''){
        var product_name = product_name.substr(0, 3);
        var random_string = generateRandomString();
        var sku = product_name.toUpperCase() + "-" + random_string;
